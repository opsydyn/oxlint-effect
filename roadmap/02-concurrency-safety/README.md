...
| [x] | `linteffect/no-yield-with-held-semaphore-permit` | lock-across-suspension discipline | strict | high | `yield* Semaphore.withPermits(...)` / acquire-permit style code whose callback contains unrelated `yield*` or forked work |
| [x] | `linteffect/no-yield-with-held-mutable-ref` | mutable-reference-across-suspension discipline | strict | high | mutable state handles captured before a later `yield*` in the same `Effect.gen` block |
| [x] | `linteffect/no-unscoped-background-fiber` | structured ownership / RAII | strict | medium | forked effects stored, returned, or discarded without scoped lifetime, supervisor, join, await, or interrupt evidence |
