...
describe("linteffect oxlint integration", () => {
  ...
  it("reports exactly the Error Modeling Slice 1 diagnostics", () => {
    const result = runOxlint(
      "error-modeling-slice-1.ts",
      "oxlint.error-modeling-slice-1.config.ts",
    );
    const ruleIds = [...result.output.matchAll(/linteffect\(([^)]+)\)/g)]
      .map((match) => match[1])
      .sort();
  ...
  it("reports exactly the Error Modeling Slice 3 diagnostics", () => {
    const result = runOxlint(
      "error-modeling-slice-3.ts",
      "oxlint.error-modeling-slice-3.config.ts",
    );
    const ruleIds = [...result.output.matchAll(/linteffect\(([^)]+)\)/g)]
      .map((match) => match[1])
      .sort();
  ...
  it("reports exactly the strict test-shape diagnostics through the CLI", () => {
    const result = runOxlint("test-shape-invalid.test.ts", "oxlint.test-shape.config.ts");
    const ruleIds = [...result.output.matchAll(/linteffect\(([^)]+)\)/g)]
      .map((match) => match[1])
      .sort();
  ...
  it("allows strict test-shape-safe Effect tests through the CLI", () => {
    const result = runOxlint("test-shape-valid.test.ts", "oxlint.test-shape.config.ts");
